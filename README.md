<p align="center">
  <img src="frontend/public/ridemitr-logo.gif" alt="RideMitr WA Logo" width="180"/>
</p>

<h1 align="center">RideMitr WA</h1>
<p align="center">
  <strong>Your own WhatsApp Business inbox & CRM — running on your own server</strong>
</p>

<p align="center">
  <a href="#-what-is-ridemitr-wa">What is it?</a> •
...
<p align="center">
  <a href="https://youtu.be/tvYR0cGOj_4"><img src="docs/ridemitr-wa-overview.gif" alt="RideMitr WA video tour" width="720" /></a>
</p>
...
**RideMitr WA** is a free WhatsApp Business inbox and CRM that **you host yourself**. Instead of paying a monthly fee to a SaaS company that keeps all your customer chats on *their* servers, you run RideMitr WA on your own server — so **you own your data and your customer conversations**.
...
> **You don't need to be a programmer to set this up.** If you can install Docker and copy-paste two commands, you can run RideMitr WA. It takes about **5 minutes**.
...
- RideMitr WA reminds you about WhatsApp's **24-hour reply rule** and suggests a template when needed
...
- Connect an **AI model** (OpenAI, Claude, Gemini, OpenRouter, OmniRoute) and let an agent **answer customers automatically**
...
3. In **Terminal**, run:
   ```bash
   git clone https://github.com/Nareshbhati2722/RideMitr-WA-FC.git
   cd RideMitr WA
   docker compose up -d
   # then print the exact address to open:
   echo "RideMitr WA is running at http://localhost:$(docker compose port ridemitr-wa-frontend 80 | cut -d: -f2)"
   ```
...
No config files, no secrets, no database commands — RideMitr WA creates its own secure keys and sets up its database automatically. The first launch builds the app, so give it a minute. Want a different port? `HTTP_PORT=9000 docker compose up -d`.
...
3. In **PowerShell**, run:
   ...
   docker compose up -d
   # then print the exact address to open:
   "RideMitr WA is running at http://localhost:$((docker compose port ridemitr-wa-frontend 80).Split(':')[-1].Trim())"
   ```
4. Open the **http://localhost:…** address the last command printed (default **http://localhost:8080**), and create your admin account right in the browser.

No config files, no secrets, no database commands — RideMitr WA sets everything up on first start. The first launch builds the app, so give it a minute. Want a different port? `$env:HTTP_PORT=9000; docker compose up -d`.
...
> ⚠️ **Keep the `secrets` volume.** RideMitr WA stores its encryption key there. If you delete it, saved WhatsApp tokens can no longer be decrypted and you'll have to reconnect WhatsApp.
...
- 🍎🪟 **Local install (Mac/Windows):** `localhost` isn't reachable from the internet, so start the **built-in Cloudflare Tunnel** — no account, nothing to install, it runs as a container. From the `RideMitr WA` folder:
  ```bash
...
### Step 2 — Add the account in RideMitr WA

1. **Log in** to RideMitr WA (at `http://localhost:8080`, or your domain) → **Settings** → **WhatsApp Accounts** → **Add**.
2. From the [Meta Business dashboard](https://business.facebook.com/) (your app → **WhatsApp → API Setup**), copy these into the form: **Phone Number ID**, **WABA ID**, **Meta App ID**, and a **Meta access token**. Also make up a **verify token** — any random text (you'll paste the *same* value into Meta in Step 3). RideMitr WA encrypts the access token and **auto-detects your business phone number and name from Meta** using that token — so make sure it's valid (a *test number*'s token from the API Setup page expires after 24 hours).

### Step 3 — Point Meta's webhook at RideMitr WA
...
- **Callback URL:** your **public URL** from Step 1, followed by `/api/webhook/whatsapp`
  ...
  - ⚠️ On a local install, **ignore the `http://localhost…` URL the RideMitr WA form shows** — Meta can't reach localhost. Use your **tunnel** address here instead.
- **Verify token:** the exact verify token you entered in RideMitr WA (Step 2).
- Click **Verify and save** — Meta calls your webhook to confirm it (RideMitr WA and the tunnel must be running).
- **Subscribe to these webhook fields** (under *WhatsApp Business Account*):
  ...
  - `smb_message_echoes` — copies of messages your team sends from the WhatsApp app, so they also appear in RideMitr WA (coexistence)

**Verify it works:** send a real WhatsApp message from your phone to the business number — it should appear in **Chats** within seconds, and your reply from RideMitr WA should arrive back on your phone.
...
```json
{
  "mcpServers": {
    "ridemitr-wa-agents": {
      "command": "node",
      "args": ["/path/to/RideMitr WA/mcp-server/src/index.js"],
      "env": {
        "RIDEMITR_WA_API_URL": "https://your-domain/api/mcp/v1",
        "RIDEMITR_WA_API_KEY": "fck_live_PASTE_YOUR_KEY"
      }
    }
  }
}
...
### Step 3 — Use it
Fully quit and reopen Claude, then say **“create a RideMitr WA agent”** — it walks you through the setup questions and builds the agent. Each capability is gated by the toggles in **Settings → MCP Tools** (a disabled one returns a clear error), and the key can be revoked any time from the same page.

> Replace `your-domain` with wherever RideMitr WA is hosted. The bearer key works over the internet; your RideMitr WA login cookie is never involved. See [`mcp-server/README.md`](mcp-server/README.md) for the full tool list and debugging tips.

---

## 🔄 Keeping it running

**Update to the latest version** (run on your server, inside the `RideMitr WA` folder):
...
0 3 * * * cd ~/RideMitr WA && docker compose exec -T ridemitr-wa-db pg_dump -U postgres postgres | gzip > ~/backups/ridemitr-wa-$(date +\%Y\%m\%d).sql.gz && find ~/backups -name '*.sql.gz' -mtime +7 -delete
```
...
| **WhatsApp connected but no chats appear** | RideMitr WA auto-detects your business number from Meta using the access token — if that token was invalid/expired when you connected, the number stays blank and chats for it are hidden (they're still saved). Fix: **Settings → WhatsApp Accounts → edit → paste a valid access token → Save** to re-fetch it. Received messages then appear immediately. |
...
| **"502" error or blank screen** | The app may still be starting. Wait a minute, then check the logs: `docker compose logs ridemitr-wa-backend`. |
...
| **Messages aren't arriving** | Re-check the **webhook** in the Meta dashboard: subscribe to `messages`, make sure the **Callback URL ends with `/api/webhook/whatsapp`**, and the **Verify token matches exactly** what you entered in RideMitr WA (no extra spaces). |
...
Still stuck? Open an issue on [GitHub](https://github.com/Nareshbhati2722/RideMitr-WA-FC/issues) and we'll help.
...
RideMitr WA is [**fair-code**](https://faircode.io) distributed under the **[Sustainable Use License](./LICENSE.md)**.
...
Copyright © 2026 **RideMitr**. **RideMitr AI** is a trademark of RideMitr — see **[TRADEMARK.md](./TRADEMARK.md)**.

> **WhatsApp** is a trademark of WhatsApp LLC. **Meta** is a trademark of Meta Platforms, Inc. RideMitr WA is an independent application that connects to the WhatsApp Cloud API (hosted by Meta), and is **not** affiliated with, endorsed by, sponsored by, or otherwise officially connected to Meta Platforms, Inc. or WhatsApp LLC.

---

<div align="center">

**RideMitr WA** — own your inbox.

<sub>Made with ❤️ by <a href="https://github.com/Nareshbhati2722">RideMitr</a></sub>
