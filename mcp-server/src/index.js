#!/usr/bin/env node
// RideMitr WA Agent Builder — MCP server.
//
// A thin, well-described client over the RideMitr WA MCP API
...
// BEFORE calling create_agent — see the `create-ridemitr-wa-agent` prompt and the
// tool descriptions.
...
const API_URL = (process.env.RIDEMITR_WA_API_URL || '').replace(/\/$/, '');
const API_KEY = process.env.RIDEMITR_WA_API_KEY || '';

if (!API_URL || !API_KEY) {
  console.error('[ridemitr-wa-mcp] RIDEMITR_WA_API_URL and RIDEMITR_WA_API_KEY env vars are required.');
  process.exit(1);
}
...
const server = new McpServer({ name: 'ridemitr-wa-agents', version: '1.0.0' });
...
server.registerTool('list_media', {
  ...
  description: 'List items from the RideMitr WA media library, optionally filtered by type and/or name. Returns [{ id, name, mediaType, mimeType }]. When the user mentions a media file by name (e.g. "use the logo image"), call this with that name to resolve it to an id automatically — then use that id in mediaGroups. Never ask the user for an id.',
  inputSchema: {
...
server.registerTool('create_agent', {
  title: 'Create agent',
  description:
    'Create a new RideMitr WA AI agent. IMPORTANT: gather and CONFIRM all settings with the user first ' +
...
const GUIDE = `You are creating a RideMitr WA WhatsApp AI agent. Gather and CONFIRM the full configuration with the user before calling create_agent. Offer real options fetched from their account — never guess ids, spreadsheets, tabs, models, or numbers.
...
server.registerPrompt('create-ridemitr-wa-agent', {
  title: 'Create a RideMitr WA agent',
  description: 'Guided flow to create and configure a RideMitr WA WhatsApp AI agent (asks the right questions, then creates it).',
  argsSchema: {},
}, () => ({
...
console.error('[ridemitr-wa-mcp] ready');
