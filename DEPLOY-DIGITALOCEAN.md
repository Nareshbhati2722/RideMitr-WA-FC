# Deploy RideMitr WA to a DigitalOcean Droplet (Ubuntu) — `ridemitr-wa.example.com`

End-to-end guide to run RideMitr WA on a fresh DigitalOcean Ubuntu droplet using Docker
...
> Repo: `https://github.com/Nareshbhati2722/RideMitr-WA-FC` (branch `main`). If your code lives in
...
|------|-----------|-------------------|-----|
| A | `ridemitr-wa` | `<droplet-ipv4>` | 3600 (or default) |

This makes `ridemitr-wa.example.com` resolve to the droplet. Verify from your laptop:

```bash
dig +short ridemitr-wa.example.com      # should print the droplet IP
```
...
```bash
cd ~
git clone https://github.com/Nareshbhati2722/RideMitr-WA-FC.git ridemitr-wa
cd ridemitr-wa
```
...
```bash
cd ~
git clone https://<YOUR_GH_USERNAME>:<YOUR_PAT>@github.com/Nareshbhati2722/RideMitr-WA-FC.git ridemitr-wa
cd ridemitr-wa
...
# Web
CORS_ORIGIN=https://ridemitr-wa.example.com
...
until [ "$(docker inspect -f '{{.State.Health.Status}}' ridemitr-wa-db)" = healthy ]; do
...
RideMitr WA keeps its data in a `coexistence` schema. The backend creates the base
`ridemitr_wa_users` table on boot; migration 031 expects it to exist first, so we create it
before applying migrations:

```bash
# 1) schema + base users table
docker compose exec -T ridemitr-wa-db psql -U postgres -d postgres <<'SQL'
...
for f in $(ls db/migrations/*.sql | sort); do
  echo ">> applying $f"
  docker compose exec -T ridemitr-wa-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 < "$f" \
...
```bash
docker compose up -d ridemitr-wa-backend ridemitr-wa-frontend caddy
docker compose ps          # all five should be Up (db/redis "healthy")
```

Caddy now requests a Let's Encrypt certificate for `ridemitr-wa.example.com` (needs DNS +
...
curl -fsS https://ridemitr-wa.example.com/api/health      # -> {"ok":true}

# Backend logs should show it booted + workers started
docker compose logs ridemitr-wa-backend | tail -n 20
```

Then open **https://ridemitr-wa.example.com** in a browser and log in with the seeded admin:

- **Email:** `admin@ridemitr.space` (override with `ADMIN_EMAIL`)
- **Password:** whatever you set in `ADMIN_PASSWORD`. If you left it unset, a random password was generated and printed once on first boot — find it with `docker compose logs ridemitr-wa-backend | grep '\[auth\]'`, then change it via Admin Settings → Team.
...
3. **Configure the Meta webhook** (Meta Business Suite → WhatsApp → Configuration):
   - **Callback URL:** `https://ridemitr-wa.example.com/api/webhook/whatsapp`
...
cd ~/ridemitr-wa
git pull
docker compose build ridemitr-wa-backend ridemitr-wa-frontend
docker compose up -d ridemitr-wa-backend ridemitr-wa-frontend
# If you pushed new SQL files, apply them:
for f in $(ls db/migrations/*.sql | sort); do
  docker compose exec -T ridemitr-wa-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 < "$f" >/dev/null 2>&1
done
```
...
0 3 * * * docker exec ridemitr-wa-db pg_dump -U postgres postgres | gzip > ~/backups/ridemitr-wa-$(date +\%Y\%m\%d).sql.gz && find ~/backups -name '*.sql.gz' -mtime +7 -delete
```

Restore (into a fresh DB) if ever needed:

```bash
gunzip -c ~/backups/ridemitr-wa-YYYYMMDD.sql.gz | docker exec -i ridemitr-wa-db psql -U postgres -d postgres
```
...
| Symptom | Fix |
|---|---|
| Caddy can't get a cert | DNS not pointing to the droplet yet (`dig +short ridemitr-wa.example.com`), or ports 80/443 blocked (`sudo ufw status`). Fix, then `docker compose restart caddy`. |
| `502`/blank page | Backend not up: `docker compose logs ridemitr-wa-backend`. Often a bad `DATABASE_URL` (must match `POSTGRES_PASSWORD`) or migrations not applied. |
| Login fails / "Invalid credentials" | Migrations applied + backend booted? The admin is seeded on first backend boot. Check `docker compose logs ridemitr-wa-backend | grep admin`. |
...
```
ridemitr-wa.example.com {
    reverse_proxy ridemitr-wa-frontend:80
}
```
...
Browser ──HTTPS──► Caddy (:443, auto-TLS)
                      │
                      ▼
            ridemitr-wa-frontend (nginx :80)
              ├─ serves the React SPA
              └─ proxies /api, /api/events, /uploads, /l/ ──► ridemitr-wa-backend (:3011)
                                                                  ├─ Postgres (ridemitr-wa-db:5432, coexistence schema)
