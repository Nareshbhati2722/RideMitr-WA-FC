...
--   * Widen ridemitr_wa_users with status, audit, and per-user permission overrides
...
ALTER TABLE coexistence.ridemitr_wa_users
  ADD COLUMN IF NOT EXISTS is_active     BOOLEAN     NOT NULL DEFAULT TRUE,
  ADD COLUMN IF NOT EXISTS permissions   JSONB       DEFAULT NULL,  -- { grant: [...], revoke: [...] } overlays on role defaults
  ADD COLUMN IF NOT EXISTS last_login_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS created_by    BIGINT      REFERENCES coexistence.ridemitr_wa_users(id) ON DELETE SET NULL;
...
ALTER TABLE coexistence.ridemitr_wa_users
  DROP CONSTRAINT IF EXISTS ridemitr_wa_users_role_check;
ALTER TABLE coexistence.ridemitr_wa_users
  ADD CONSTRAINT ridemitr_wa_users_role_check
...
ALTER TABLE coexistence.contacts
  ADD COLUMN IF NOT EXISTS assigned_user_id BIGINT REFERENCES coexistence.ridemitr_wa_users(id) ON DELETE SET NULL;
...
CREATE TABLE IF NOT EXISTS coexistence.user_audit_log (
  id              BIGSERIAL PRIMARY KEY,
  actor_user_id   BIGINT REFERENCES coexistence.ridemitr_wa_users(id) ON DELETE SET NULL,
  actor_username  TEXT,
