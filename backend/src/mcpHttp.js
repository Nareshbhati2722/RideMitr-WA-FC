...
function buildServer(capabilities) {
  ...
  server.registerTool('create_agent', {
    title: 'Create agent',
    description:
      'Create a new RideMitr WA AI agent. Gather + CONFIRM all settings with the user first. For an ACTIVE agent pass aiModelId + llmModel; otherwise status:"draft". Only one active agent per WhatsApp number. After creating, use add_google_sheets_tool to attach a Sheets tool if wanted.',
    inputSchema: {
      name: z.string(),
  ...
  server.registerPrompt('create-ridemitr-wa-agent', {
    ...
    description: 'Guided flow to create and configure a RideMitr WA WhatsApp AI agent.',
    argsSchema: {},
